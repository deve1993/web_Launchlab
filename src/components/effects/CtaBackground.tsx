      ...
      .map(() =>
        Array(cols)
          ...
          .map(() => ({
            ...
            opacity: Math.random() > 0.82 ? 0.6 : 0,
          })),
      )
      ...
      for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
          ...
          if (cell.alive && cell.opacity < 1) {
            ...
            cell.opacity = Math.max(cell.opacity - transitionSpeed * 0.5, 0)
          }

          if (cell.opacity > 0) {
            if (cell.opacity > 0.45) {
              ctx.fillStyle = `rgba(249, 115, 22, ${cell.opacity * 0.4})`
            } else {
              ctx.fillStyle = `rgba(0, 0, 0, ${cell.opacity * 0.85})`
            }
            ctx.beginPath()
            ctx.arc(
